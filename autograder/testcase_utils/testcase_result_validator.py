...
def generate_validating_string() -> str:
    ...
    return "".join(
        random.choice(POSSIBLE_CHARS_IN_VALIDATING_STRING)
        for _ in range(VALIDATING_STRING_LENGTH)
    )


# Beware: Horrible space and time complexity. Maybe we should optimize that?
def validate_output(
    output: str, required_validating_string: str
    ...
    try:
        result, actual_validating_string = last_line.split(
            LAST_LINE_SPLITTING_CHARACTER, 1
        )
        result = float(result)
